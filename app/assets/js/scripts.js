document.addEventListener("DOMContentLoaded", function () {
    ...
    function getWorkshopsData() {
        ...
        const raw = sectionFiltre ? sectionFiltre.getAttribute("data-workshops") : "[]";
    ...
    document.querySelectorAll(".close-modal, .btn-cancel, .close-flash-modal, .close-confirm").forEach(btn => {
        btn.onclick = () => {
    ...
    if (selectCategorie) {
        selectCategorie.addEventListener("change", function () {
            ...
            const filtered = workshops.filter(w => String(w.typeId) === String(this.value));
    ...
    if (selectSport) {
        selectSport.addEventListener("change", function () {
            ...
            selectDate.innerHTML = '<option value="" selected disabled>Date et heure</option>';
            const creneaux = workshops.filter(w => w.name === nomSportSelectionne);
    ...
    if (sectionFiltre && selectCategorie && selectSport) {
        ...
        if (idASelectionner && idASelectionner !== "null") {
            const workshops = getWorkshopsData();
            const monCours = workshops.find(w => String(w.id) === String(idASelectionner));
